...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${PROJECT_ROOT}/checkpoints/rsprompter-fusion-satS-v1}"
...
LOG_FILE="${LOG_DIR}/rsprompter_fusion_satS_v1_${TIMESTAMP}.log"
mkdir -p "$LOG_DIR"
ln -sf "$LOG_FILE" "${LOG_DIR}/rsprompter_fusion_latest_satS_v1.log"

CONFIG_PATH="${PROJECT_ROOT}/configs/rsprompter_anchor_satS_drone_guidance_v1.py"
TRAIN_BIN="${PROJECT_ROOT}/train/train_rsprompter_fusion.py"

echo "========================================"
echo "RSPrompter(SAM) 卫星主分支(S) - 无人机语义引导 融合训练"
echo "========================================"
echo "训练参数:"
echo "  - 总轮数: $EPOCHS"
echo "  - 学习率: $LEARNING_RATE"
echo "  - 批大小: $BATCH_SIZE (梯度累积: $GRAD_ACCUM_STEPS)"
echo "  - LR倍率: backbone=${SAT_BACKBONE_LR_MULT} other=${SAT_OTHER_LR_MULT} drone=${DRONE_LR_MULT} scene_align=${SCENE_ALIGN_LR_MULT}"
