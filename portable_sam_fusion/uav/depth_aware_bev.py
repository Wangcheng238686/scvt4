...
class DepthAwareCVTEncoder(nn.Module):
    """
    Depth-aware Cross-View Transformer Encoder.
    ...
    def forward(
        ...
        if self.use_depth and depth is not None:
            height_map = self._compute_height_map(depth, extrinsics)
    ...
    def _compute_height_map(
        ...
        height_flat = depth_flat
        
        height = rearrange(height_flat, "(b n) c h w -> b n c h w", b=b, n=n)
        
        height_avg = height.mean(dim=1)
        
        bev_h = self.bev_embedding.h
        bev_w = self.bev_embedding.w
        height_map = F.interpolate(height_avg, size=(bev_h, bev_w), mode='bilinear', align_corners=False)
