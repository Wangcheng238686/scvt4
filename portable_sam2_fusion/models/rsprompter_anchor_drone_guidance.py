...
class RSPrompterAnchorDroneGuidance(RSPrompterAnchor):
    ...
    def extract_feat(
    ...
    ) -> Tuple[Tuple[torch.Tensor, ...], torch.Tensor, torch.Tensor, Optional[List[torch.Tensor]]]:
        ...
        if self.guidance is not None:
            if self.use_height_guided_fusion:
                guided_feats, align_loss, height_loss, height_loss_dict = self.guidance(
                    feats=x, 
                    bev_feat=drone_bev,
                ...
                self._height_loss_dict = height_loss_dict
    ...
    def loss(
        ...
        if not self.use_cross_view_loss:
            return self._compute_standard_loss(x, image_embeddings, image_positional_embeddings, batch_data_samples, high_res_features)
        
        return self._compute_loss_with_cross_view(x, image_embeddings, image_positional_embeddings, batch_data_samples, high_res_features)
